...
<div id="container" class=“fontsize”>
	...
	<div id="body">		
		...
		if( $tradesignal && $endtrade==0 ){
			if($newtime != 0)
				echo '现在是第'.$turnno.'轮交易阶段，倒计时<input type="text" id="time" size="2" onfocus="this.blur();"> 秒(本页面每5秒自动刷新一次)'; 
			else echo '第'.$turnno.'轮交易即将结束，请将现场交易员与采购员请回大本营。';
		}
		else if( $endtrade ) echo '第'.$turnno.'轮交易结束，现在进入黄金合成阶段';
		else echo '第'.$turnno.'轮交易即将开始'?>
